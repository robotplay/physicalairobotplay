...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e0e0e0; border-top: none;">
        <p style="font-size: 16px; margin-bottom: 20px;">
            안녕하세요, <strong>${escapeHtml(data.name)}</strong>님!
        </p>
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #00A3FF;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                    ...
                    <td style="padding: 8px 0;">${escapeHtml(courseName)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold; vertical-align: top;">문의 내용:</td>
                    <td style="padding: 8px 0;">${escapeHtml(data.message).replace(/\n/g, '<br>')}</td>
                </tr>
            </table>
        </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                    ...
                    <td style="padding: 8px 0;">
                        <a href="tel:${escapeHtml(data.phone)}" style="color: #00A3FF; text-decoration: none;">${escapeHtml(data.phone)}</a>
                    </td>
                </tr>
                ${data.email ? `
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">이메일:</td>
                    <td style="padding: 8px 0;">
                        <a href="mailto:${escapeHtml(data.email)}" style="color: #00A3FF; text-decoration: none;">${escapeHtml(data.email)}</a>
                    </td>
                </tr>
                ` : ''}
                <tr>
...
// 관리자 알림 이메일 템플릿 (특강신청)
export function createAdminRegistrationNotificationTemplate(data: {
    studentName: string;
    grade: string;
    parentName: string;
    phone: string;
    ...
    const subject = `[특강신청] ${data.programName} - ${data.studentName}(${data.grade}) 신청 접수`;
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #FF4D4D 0%, #00A3FF 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="color: white; margin: 0; font-size: 24px;">새로운 특강신청</h1>
    </div>
    
    <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e0e0e0; border-top: none;">
        <p style="font-size: 16px; margin-bottom: 20px;">
            새로운 특강신청이 접수되었습니다. 관리자 페이지에서 확인해주세요.
        </p>
        
        <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #FF4D4D;">
            <h2 style="margin-top: 0; color: #FF4D4D; font-size: 18px;">신청 정보</h2>
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 8px 0; font-weight: bold; width: 120px;">프로그램:</td>
                    <td style="padding: 8px 0;">${escapeHtml(data.programName)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">학생 이름:</td>
                    <td style="padding: 8px 0;">${escapeHtml(data.studentName)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">학년:</td>
                    <td style="padding: 8px 0;">${escapeHtml(data.grade)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">보호자 이름:</td>
                    <td style="padding: 8px 0;">${escapeHtml(data.parentName)}</td>
                </tr>
                <tr>
                    <td style="padding: 8px 0; font-weight: bold;">연락처:</td>
                    <td style="padding: 8px 0;">
        ...
        <div style="background: #d1ecf1; padding: 15px; border-radius: 8px; border-left: 4px solid #0c5460; margin-top: 20px;">
            <p style="margin: 0; font-size: 14px; color: #0c5460;">
                <strong>다음 단계:</strong><br>
                관리자 페이지에서 특강신청을 확인하고 신청자에게 연락해주세요.
            </p>
        </div>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e0e0e0; border-top: none;">
        <p style="font-size: 16px; margin-bottom: 20px;">
            안녕하세요, <strong>${escapeHtml(data.parentName)}</strong>님!
        </p>
        
        <p style="font-size: 16px; margin-bottom: 30px;">
            <strong>${escapeHtml(data.programName)}</strong> 신청이 성공적으로 접수되었습니다.
        </p>
