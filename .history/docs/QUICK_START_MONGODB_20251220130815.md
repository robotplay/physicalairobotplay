# MongoDB Atlas 빠른 시작 가이드

## 🚀 5분 안에 MongoDB 연결하기

### 1단계: MongoDB Atlas 계정 생성 (2분)
...
2. **"M0 FREE"** 선택 (완전 무료)
3. 클라우드 제공자: **AWS** 선택
4. 리전: **Seoul (ap-northeast-2)** 또는 가장 가까운 지역 선택
5. 클러스터 이름: `academy-cluster` (또는 원하는 이름)
6. **"Create"** 클릭 (약 1-3분 소요)
...
2. **"Add IP Address"** 클릭
3. **"Allow Access from Anywhere"** 선택 (또는 `0.0.0.0/0` 입력)
4. **"Confirm"** 클릭
...
5. 연결 문자열 복사
   - 예: `mongodb+srv://academy-admin:<password>@academy-cluster.xxxxx.mongodb.net/?retryWrites=true&w=majority`
6. **`<password>` 부분을 실제 비밀번호로 교체**
   - 예: `mongodb+srv://academy-admin:MyPassword123@academy-cluster.xxxxx.mongodb.net/?retryWrites=true&w=majority`
...
```env
MONGODB_URI=mongodb+srv://academy-admin:MyPassword123@academy-cluster.xxxxx.mongodb.net/?retryWrites=true&w=majority
MONGODB_DB_NAME=academy-site
```

**⚠️ 중요:** 
- `academy-admin`을 실제 사용자 이름으로 교체
- `MyPassword123`을 실제 비밀번호로 교체
...
2. **신청서 제출 테스트:**
   ...
   - 신청서 제출
   - 브라우저 콘솔에서 `✅ MongoDB 저장 성공` 메시지 확인
