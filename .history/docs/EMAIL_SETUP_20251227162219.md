...
ADMIN_EMAIL=admin@parplay.co.kr (선택사항, 관리자 알림 수신 이메일)
```
...
- **ADMIN_EMAIL**: 관리자 알림 수신 이메일 (선택사항)
  - 상담문의 신청 시 관리자에게 알림 이메일이 발송됩니다
  - 설정하지 않으면 `SMTP_FROM` 또는 `SMTP_USER`를 사용합니다
  - 예: `admin@parplay.co.kr`
