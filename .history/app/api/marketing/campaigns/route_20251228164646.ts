    ...
    try {
        ...
        if (type === 'newsletter') {
            emailTemplate = createNewsletterEmailTemplate({
                title,
                content,
                unsubscribeUrl: `${siteUrl}/newsletter/unsubscribe?email={email}`,
            });
            ...
            return NextResponse.json(
                { success: false, error: '유효하지 않은 캠페인 타입입니다.' },
                { status: 400 }
            );
        }
        ...
        sendBulkEmail(recipients, emailTemplate.subject, emailTemplate.html)
            .then(async (result) => {
                // 캠페인 상태 업데이트
            ...
            .catch(async (error) => {
                ...
                console.error(`캠페인 발송 실패 (${campaignId}):`, error);
            });
