...
export default function PaymentButton({
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            // PortOne.requestPayment()를 직접 호출하는 방식
            // macOS 호환성을 위해 payMethod를 지정하지 않음 (포트원이 Channel 설정에 따라 자동 선택)
            // portone은 PortOne 모듈이므로 portone.requestPayment()로 호출
            const response = await portone.requestPayment({
                ...
                totalAmount: amount,
                currency: 'KRW',
                // payMethod를 지정하지 않으면 포트원이 Channel에 설정된 결제 수단을 사용
                // macOS에서는 HTML5_INICIS가 지원되지 않으므로 payMethod 제거
                customer: {
