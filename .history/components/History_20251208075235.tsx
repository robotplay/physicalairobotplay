...
export default function History() {
    const events = [
        ...
        },
        {
            year: "2020",
            title: "메이커 대회 수상",
            description: "전국 청소년 메이커 대회 대상 수상. 우리의 교육 방식이 틀리지 않았음을 증명한 순간이었습니다.",
            icon: Award,
        },
        {
            year: "2023",
                        ...
                        {events.map((event, index) => {
                            ...
                            return (
                                <ScrollAnimation
                                    ...
                                    delay={index * 200}
                                >
                                    <div className={`relative flex items-center group ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
                                        {/* Content */}
                                        <div className="w-1/2 px-8">
                                            <div className={`text-${index % 2 === 0 ? 'right' : 'left'}`}>
                                                <span className="text-6xl font-black text-gray-100 dark:text-gray-800 absolute -top-10 opacity-50 z-0 select-none transition-all group-hover:opacity-70 group-hover:scale-110">
                                                    {event.year}
                                                </span>
                                                <div className="relative z-10 transform transition-all group-hover:scale-105">
                                                    <div className="inline-flex items-center gap-2 mb-3">
                                                        <Icon className="w-6 h-6 text-deep-electric-blue" />
                                                        <h4 className="text-2xl font-bold text-deep-electric-blue">{event.title}</h4>
                                                    </div>
                                        ...
                                        {/* Animated Dot */}
                                        <div className="absolute left-1/2 transform -translate-x-1/2 z-20 group">
